...
void main()
{
    ...
    // Ambient component
    float ambient_component = u_material_ambient;

    // Diffuse component
    float light_dot_norm = dot(light_vector, v_normal);
    float diffuse_componenet = light_dot_norm * u_material_diffuse;
    ...
    float specular_component = pow(reflect_dot_view, u_material_shininess) * u_material_specular;
